  ...
  return (
    <div className={styles.mapForm}>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Город" placeholder="Начните вводить город..." />}
      ...
      <Autocomplete
        ...
        style={{ width: 300 }}
        getOptionSelected={(option, value) => option.id === value.id}
        renderInput={(params) => <TextField {...params} label="Пункт выдачи" placeholder="Начните вводить пункт..." />}
        noOptionsText='Ничего не найдено'
        onChange={(event, value, reason) => {
          if (reason === "select-option") {
            setCenter({ lat: value.lat, lng: value.lng });
